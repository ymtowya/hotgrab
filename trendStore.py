...
class TrendStore:
    ...
    def logTable(self):
        if len(self.date) > 4:
            ...
            sqlStrLogTable = "INSERT INTO `{dbname}`.`recorddates` (`year`, `date`) \
                                VALUES ({year}, {date});"\
                                .format(dbname=self.dbname, year=year, date=date)
            try:
                with self.mydb.cursor() as curs:
    ...
    def insertTrends(self, trends, time):
        ...
        with self.mydb.cursor() as curs:
            try:
                for trend in trends:
                    sqlStrInsertTrend = "INSERT INTO `{dbname}`.`{tbname}` \
                                        (`position`, `title`, `views`, `time`) \
                                        VALUES ({position}, '{title}', {views}, {time});"\
                                        .format(dbname=self.dbname, tbname=self.getTbName(), \
                                            position=trend.getPosition(), \
